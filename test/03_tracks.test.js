...
const { UsersMoldel, StorageMoldel, TracksMoldel } = require("../models");
const {
...
beforeAll(async () => {
  await UsersMoldel.deleteMany({});
  await StorageMoldel.deleteMany({});
  const user = await UsersMoldel.create(testAuthRegisterAdmin);
  const storage = await StorageMoldel.create(testStorageRegister);
...
test("debe retornar todo el detalle del item", async () => {
  const { _id } = await TracksMoldel.findOne({});
  id = _id.toString();
  const res = await request(app)
    .get(`/api/tracks/${id}`)
    .set("Authorization", `Bearer ${JWT_TOKEN}`);
